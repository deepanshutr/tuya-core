...
app = typer.Typer(no_args_is_help=True, help="tuya-core operator CLI")


@app.command()
def serve(bind: str = typer.Option(None, "--bind", help="Override TUYA_BIND")) -> None:
    """Run the HTTP daemon."""
...
def discover() -> None:
    ...
    async def go() -> None:
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.post(url, json={"passive": True})
            r.raise_for_status()
