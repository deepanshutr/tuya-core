"""Tests for the ESP-TOUCH onboarding handler (tuya_core.onboard)."""
...
from tuya_core.onboard import OnboardResult, onboard
...
async def test_onboard_returns_joined_bulbs(tmp_path: Path, mocker) -> None:
    ...
    async def fake_run(ssid, password, *, timeout_s, on_join, bssid, local_ip, **kw):
        ...
        reg.upsert_discovered({"mac": "d8a0118dc5c3", "ip": "192.168.1.9"})
        return await on_join()

    mocker.patch("tuya_core.onboard.esptouch_run", new=fake_run)
...
async def test_onboard_only_reports_newly_joined_macs(tmp_path: Path, mocker) -> None:
    ...
    reg = _registry(tmp_path)
    reg.upsert_discovered({"mac": "aaaaaaaaaaaa", "ip": "192.168.1.2"})
...
async def test_onboard_internal_error_on_esptouch_failure(
    ...
    mocker.patch("tuya_core.onboard.esptouch_run", new=boom)
...
def test_onboard_result_carries_newbulb_fields() -> None:
    ...
    bulbs = [NewBulb(mac="d8a0118dc5c3", ip="192.168.1.9", name="bulb-1", rssi=-55)]
    result = OnboardResult.from_bulbs(bulbs)
    assert result.status == "ok"
    assert result.onboarded == [
        {"mac": "d8a0118dc5c3", "ip": "192.168.1.9", "name": "bulb-1", "rssi": -55}
    ]
